...
def train_discriminator(real_images,real_classes , discriminator_optimizer,discriminator,generator,batch_size,latent_size,described_species,device,n_classes,species2prob):
    ...
    #SELECT RARE CLASSES MORE OFTEN
    random_classes = torch.tensor(random.choices(list(range(0,n_classes)),weights=species2prob,k=batch_size),device=device)
...
def train_generator(generator_optimizer,generator,discriminator,batch_size,latent_size,described_species,device,n_classes,species2prob):
    ...
    #random_classes = torch.tensor(described_species[np.random.randint(0, len(described_species), batch_size)],device=device) OLD
    ...
    return loss.item()
def fit(epochs,fit_p : Fit_params, save_p : Save_samples_params,species2prob,start_idx=0,latent_size =100):
    ...
    for epoch in range(epochs):
        for real_images, real_classes in tqdm(fit_p.dataloaders['train']):
            ...
            loss_d, real_score, fake_score, class_accuracy_real, class_accuracy_fake = train_discriminator(real_images, real_classes,fit_p.discriminator_optimizer,fit_p.discriminator,fit_p.generator,fit_p.batch_size,fit_p.latent_size,fit_p.described_species_labels,fit_p.device,fit_p.n_classes,species2prob)
            # Train generator FOR MORE TIMES THAN DISCRIMINATOR
            #for k in range(3):
            loss_g = train_generator(fit_p.generator_optimizer,fit_p.generator,fit_p.discriminator,fit_p.batch_size,fit_p.latent_size,fit_p.described_species_labels,fit_p.device,fit_p.n_classes,species2prob)
