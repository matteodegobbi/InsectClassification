...
'''class Discriminator(torch.nn.Module):
    ...
    def __init__(self,n_feature_maps=10,n_classes=1050):
        super(Discriminator, self).__init__()
        #self.convstride = nn.Sequential(nn.Conv2d(3,n_feature_maps,6,2,1,bias=False,),
        ...
        self.conv5 = nn.Sequential(nn.Conv2d(n_feature_maps*8,n_feature_maps*16,4,2,1,bias=False),
                                   ...
                                   nn.LeakyReLU(0.2,inplace=True),
                                   nn.Dropout(0.5))
        self.conv6 = nn.Sequential(nn.Conv2d(n_feature_maps*16,n_feature_maps*32,4,2,0,bias=False),
                                   nn.BatchNorm2d(n_feature_maps*32),
                                   nn.LeakyReLU(0.2,inplace=True),
                                   nn.Dropout(0.5))
        
        self.conv7 = nn.Sequential(nn.Conv2d(n_feature_maps*32,n_feature_maps*32,4,2,0,bias=False),
                                   nn.BatchNorm2d(n_feature_maps*32),
                                   nn.LeakyReLU(0.2,inplace=True),
                                   nn.Dropout(0.5))
        self.main = nn.Sequential(#self.convstride,
            self.conv1,
                                  self.conv2,
                                  self.conv3,        
                                  self.conv4,
        ...
        lin = 8000
        self.linear_realfake = nn.Linear(lin,1)
        self.linear_class = nn.Linear(lin,n_classes)
...
class Generator(nn.Module):
    def __init__(self, n_feature_maps=512,noise_size=100,n_classes=1050,embedding_size=200):
        ...
        self.main = nn.Sequential(
            ...
            #Tpose(n_feature_maps:=n_feature_maps//2,3),
            #ResizeConvLayer(n_feature_maps//2,3,3,1),
            #ResizeConvLayer(n_feature_maps:=n_feature_maps//2),
            #ResizeConvLayer(n_feature_maps:=n_feature_maps//2),
            #ResizeConvLayer(n_feature_maps:=n_feature_maps//2),
            #ResizeConvLayer(n_feature_maps:=n_feature_maps//2,kernel_size=6),
            #ResizeConvLayer(n_feature_maps:=n_feature_maps//2,kernel_size=6,padding=1),
            #ResizeConvLayer(n_feature_maps:=n_feature_maps//2,kernel_size=7),
            #nn.Conv2d(n_feature_maps//2,out_channels=3,kernel_size=3,stride=3,padding=0),
...
class Generator(nn.Module):
    def __init__(self, n_feature_maps=64,noise_size=100):
        super(Generator, self).__init__()
        self.embed = nn.Embedding(1050,500)
        self.main = nn.Sequential(
            # input is Z, going into a convolution
            nn.ConvTranspose2d( noise_size+500, n_feature_maps * 8, 4, 1, 0, bias=False),
            nn.BatchNorm2d(n_feature_maps * 8),
            nn.ReLU(True),
...
def train_discriminator(real_images,real_classes , discriminator_optimizer,discriminator,generator,batch_size,latent_size,device,n_classes):
    ...
    real_wass_loss = real_preds.mean(0).view(1)
    real_class_loss = nn.functional.nll_loss(real_predicted_classes,real_classes)
    one = torch.FloatTensor([1])
    one = one.cuda()
    mone = -one
    mone = mone.cuda()
    real_class_loss.backward(retain_graph=True)
    real_wass_loss.backward(one)
    ...
    #fake_preds_targets = torch.ones(fake_images.size(0),1, device=device)
    fake_preds,fake_predicted_classes = discriminator(fake_images)
    fake_wass_loss = fake_preds.mean(0).view(1)
    fake_class_loss = nn.functional.nll_loss(fake_predicted_classes,random_classes)
    fake_wass_loss.backward(mone,retain_graph=True)
    fake_class_loss.backward()
    ...
    return (fake_wass_loss-real_wass_loss).item(), real_score, fake_score, class_accuracy_real, class_accuracy_fake
    
def train_generator(generator_optimizer,generator,discriminator,batch_size,latent_size,device,n_classes):
